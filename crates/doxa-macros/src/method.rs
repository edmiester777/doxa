...
pub fn expand_operation(args: TokenStream, item: TokenStream) -> Result<TokenStream> {
    ...
    let sites = grant::rewrite(&mut item_fn, &parsed.method.to_string(), &path_names)?;
